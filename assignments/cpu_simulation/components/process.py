...
#io_state is a flag for storing the process start time when it gets Cpu for the first time 
        ...
        if 'priority' in kwargs:
            self.priority = kwargs['priority']
        if 'time' in kwargs:
            self.arrival_time = kwargs['time']
        if 'process_id' in kwargs:
            self.process_id = kwargs['process_id']
        self.acct['total_burst_time'] = int(kwargs['burst_time'])
    #	print (self.acct)
    ...
    def __str__(self):
        ...
        """
        return "[process_id: %s, state: %s, mem_required: %s, arrival_time: %s,burst_time: %s, priority: %s]" % (self.process_id,
                                                                              self.state,
                                                                              self.mem_required,
                                                                              self.arrival_time,
                                                                              self.burst_time,
